...
impl Client {
    ...
    pub fn id(&self) -> u64 { self.id.load(SeqCst) }
    pub fn http_client(&self) -> Arc<HttpClientWithUrl> { self.http.clone() }
    pub fn credentials_provider(&self) -> Arc<dyn credentials_provider::CredentialsProvider> {
        self.credentials_provider.provider.clone()
    }
    pub fn cloud_client(&self) -> Arc<cloud_api_client::CloudApiClient> {
    ...
    pub fn global(cx: &App) -> Arc<Self> { cx.global::<GlobalClient>().0.clone() }
    pub fn set_global(client: Arc<Client>, cx: &mut App) { cx.set_global(GlobalClient(client)) }
    ...
    pub fn status(&self) -> watch::Receiver<Status> { self.state.read().status.1.clone() }
    ...
    pub fn telemetry(&self) -> &Arc<crate::telemetry::Telemetry> { &self.telemetry }
}
