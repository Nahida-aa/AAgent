...
impl WorkspaceError for &'static str {
    fn primary_message(&self) -> SharedString { self.to_string().into() }
...
impl WorkspaceError for String {
    fn primary_message(&self) -> SharedString { self.clone().into() }
...
impl WorkspaceError for anyhow::Error {
    fn primary_message(&self) -> SharedString { format!("{self}").into() }
...
impl WorkspaceError for PortalError {
    fn primary_message(&self) -> SharedString { self.message.clone().into() }
