...
type StreamResponseChannels =
    Arc<Mutex<HashMap<MessageId, mpsc::UnboundedSender<(Result<Envelope>, oneshot::Sender<()>)>>>>;
...
impl<T: Send + Clone + 'static> Signal<T> {
    ...
    fn wait(&self) -> Shared<Task<Option<T>>> { self.rx.clone() }
}
...
pub struct ChannelClient {
    ...
    pub(crate) stream_response_channels: StreamResponseChannels,
...
impl ChannelClient {
    ...
    pub(crate) fn wait_for_remote_started(&self) -> Shared<Task<Option<()>>> {
    ...
    pub(crate) async fn resync(&self, timeout: Duration) -> Result<()> {
        smol::future::or(
            async {
    ...
    pub(crate) fn send<T: EnvelopedMessage>(&self, payload: T) -> Result<()> {
    ...
    pub(crate) fn request_stream_dynamic(
        &self,
        mut envelope: proto::Envelope,
        type_name: &'static str,
...
impl ProtoClient for ChannelClient {
    ...
    fn message_handler_set(&self) -> &Mutex<ProtoMessageHandlerSet> { &self.message_handlers }

    fn is_via_collab(&self) -> bool { false }

    fn has_wsl_interop(&self) -> bool { self.has_wsl_interop }
}
