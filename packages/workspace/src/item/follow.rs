...
use crate::{CollaboratorId, ViewId};
...
pub trait FollowableItem: Item {
    ...
    fn from_state_proto(
        project: Entity<crate::Workspace>,
        id: ViewId,
...
impl<T: FollowableItem> FollowableItemHandle for Entity<T> {
    ...
    fn downgrade(&self) -> Box<dyn WeakFollowableItemHandle> { Box::new(self.downgrade()) }
...
impl<T: FollowableItem> WeakFollowableItemHandle for WeakEntity<T> {
    fn upgrade(&self) -> Option<Box<dyn FollowableItemHandle>> { Some(Box::new(self.upgrade()?)) }
}
