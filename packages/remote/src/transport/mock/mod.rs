...
impl SendableCx {
    pub(crate) fn new(cx: &TestAppContext) -> Self { Self(cx.to_async()) }

    pub(crate) fn get(&self, _: &AsyncApp) -> AsyncApp { self.0.clone() }
}
...
impl RemoteConnection for MockRemoteConnection {
    ...
    fn path_style(&self) -> PathStyle { PathStyle::local() }
    ...
    fn remote_os_version(&self) -> Option<String> { None }

    fn shell(&self) -> String { "sh".to_owned() }

    fn default_system_shell(&self) -> String { "sh".to_owned() }
