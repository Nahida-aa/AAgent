...
impl RemoteConnection for DockerExecConnection {
    ...
    async fn kill(&self) -> Result<()> { self.kill_inner() }

    fn has_been_killed(&self) -> bool { self.proxy_process.lock().is_none() }
    ...
    fn path_style(&self) -> PathStyle { self.path_style.unwrap_or(PathStyle::Unix) }
    ...
    fn default_system_shell(&self) -> String { String::from("/bin/sh") }
}
