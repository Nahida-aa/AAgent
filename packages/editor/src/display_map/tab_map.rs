...
impl std::ops::Deref for TabSnapshot {
    ...
    fn deref(&self) -> &Self::Target { &self.fold_snapshot }
}
...
impl TabSnapshot {
    ...
    #[a_tracing::instrument(skip_all)]
    pub fn rows(&self, row: u32) -> fold_map::FoldRows<'_> { self.fold_snapshot.row_infos(row) }
    ...
    #[a_tracing::instrument(skip_all)]
    pub fn tab_point_cursor(&self) -> TabPointCursor<'_> { TabPointCursor { this: self } }
...
impl TabPointCursor<'_> {
    ...
    #[a_tracing::instrument(skip_all)]
    pub fn map(&mut self, point: FoldPoint) -> TabPoint { self.this.fold_point_to_tab_point(point) }
}
...
impl From<Point> for TabPoint {
    fn from(point: Point) -> Self { Self(point) }
}
...
impl<'a> TabStopCursor<'a> {
    ...
    fn byte_offset(&self) -> u32 { self.byte_offset }

    fn char_offset(&self) -> u32 { self.char_offset }
}
