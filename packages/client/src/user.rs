...
impl Ord for User {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering { self.username.cmp(&other.username) }
}
...
impl UserStore {
    ...
    pub fn clear_cache(&mut self) { self.users.clear(); }
    ...
    pub fn contacts(&self) -> &[Arc<Contact>] { &self.contacts }
    ...
    pub fn incoming_contact_requests(&self) -> &[Arc<User>] { &self.incoming_contact_requests }

    pub fn outgoing_contact_requests(&self) -> &[Arc<User>] { &self.outgoing_contact_requests }
    ...
    pub fn current_user(&self) -> Option<Arc<User>> { self.current_user.borrow().clone() }
    ...
    pub fn organizations(&self) -> &Vec<Arc<Organization>> { &self.organizations }
