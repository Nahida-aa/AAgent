...
impl IntoGpui for FontWeightContent {
    ...
    fn into_gpui(self) -> Self::Output { FontWeight(self.0.clamp(100., 950.)) }
}

impl IntoGpui for FontFeaturesContent {
    type Output = FontFeatures;

    fn into_gpui(self) -> Self::Output { FontFeatures(Arc::new(self.0.into_iter().collect())) }
}
...
impl IntoGpui for FontSize {
    ...
    fn into_gpui(self) -> Self::Output { px(self.0) }
}
...
impl IntoGpui for FontFamilyName {
    ...
    fn into_gpui(self) -> Self::Output { SharedString::from(self.0) }
}
