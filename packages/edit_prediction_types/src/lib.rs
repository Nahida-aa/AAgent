...
impl PredictedCursorPosition {
    pub fn new(anchor: language::Anchor, offset: usize) -> Self { Self { anchor, offset } }

    /// Creates a predicted cursor position at an exact anchor location (offset = 0).
    pub fn at_anchor(anchor: language::Anchor) -> Self { Self { anchor, offset: 0 } }
}
...
impl DataCollectionState {
    pub fn is_supported(&self) -> bool { !matches!(self, DataCollectionState::Unsupported) }

    pub fn is_enabled(&self) -> bool { matches!(self, DataCollectionState::Enabled { .. }) }
...
pub trait EditPredictionDelegate: 'static + Sized {
    ...
    fn usage(&self, _cx: &App) -> Option<EditPredictionUsage> { None }

    fn can_toggle_data_collection(&self, _cx: &App) -> bool { true }
    ...
    T: EditPredictionDelegate,
{
    ...
    fn display_name(&self) -> &'static str { T::display_name() }

    fn show_predictions_in_menu(&self) -> bool { T::show_predictions_in_menu() }

    fn show_tab_accept_marker(&self) -> bool { T::show_tab_accept_marker() }

    fn supports_jump_to_edit(&self) -> bool { T::supports_jump_to_edit() }

    fn icons(&self, cx: &App) -> EditPredictionIconSet { self.read(cx).icons(cx) }

    fn data_collection_state(&self, cx: &App) -> DataCollectionState {
        self.read(cx).data_collection_state(cx)
    }

    fn usage(&self, cx: &App) -> Option<EditPredictionUsage> { self.read(cx).usage(cx) }
    ...
    fn is_refreshing(&self, cx: &App) -> bool { self.read(cx).is_refreshing(cx) }
    ...
    fn accept(&self, cx: &mut App) { self.update(cx, |this, cx| this.accept(cx)) }
