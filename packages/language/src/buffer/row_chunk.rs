...
impl RowChunks {
    ...
    pub fn version(&self) -> &clock::Global { self.buffer_snapshot.version() }

    pub fn len(&self) -> usize { (self.last_row / self.max_rows_per_chunk) as usize + 1 }
...
impl RowChunk {
    pub fn row_range(&self) -> Range<BufferRow> { self.start..self.end_exclusive }

    pub fn anchor_range(&self) -> Range<Anchor> { self.start_anchor..self.end_anchor }
}
