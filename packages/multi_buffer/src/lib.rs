...
impl MultiBufferDiffHunk {
    pub fn status(&self) -> DiffHunkStatus { self.status }
...
impl ops::Add<usize> for MultiBufferRow {
    ...
    fn add(self, rhs: usize) -> Self::Output { MultiBufferRow(self.0 + rhs as u32) }
}
...
impl MultiBufferDimension for Point {
    ...
    fn from_summary(summary: &MBTextSummary) -> Self { summary.lines }

    fn add_text_dim(&mut self, other: &Self::TextDimension) { *self += *other; }

    fn add_mb_text_summary(&mut self, summary: &MBTextSummary) { *self += summary.lines; }
}

// todo(lw): MultiBufferPointUtf16
impl MultiBufferDimension for PointUtf16 {
    type TextDimension = PointUtf16;
    fn from_summary(summary: &MBTextSummary) -> Self { summary.lines_utf16() }

    fn add_text_dim(&mut self, other: &Self::TextDimension) { *self += *other; }

    fn add_mb_text_summary(&mut self, summary: &MBTextSummary) { *self += summary.lines_utf16(); }
}
...
impl MultiBufferDimension for MultiBufferOffset {
    ...
    fn from_summary(summary: &MBTextSummary) -> Self { summary.len }

    fn add_text_dim(&mut self, other: &Self::TextDimension) { self.0 += *other; }

    fn add_mb_text_summary(&mut self, summary: &MBTextSummary) { *self += summary.len; }
}
impl MultiBufferDimension for MultiBufferOffsetUtf16 {
    type TextDimension = OffsetUtf16;
    fn from_summary(summary: &MBTextSummary) -> Self { MultiBufferOffsetUtf16(summary.len_utf16) }

    fn add_text_dim(&mut self, other: &Self::TextDimension) { self.0 += *other; }

    fn add_mb_text_summary(&mut self, summary: &MBTextSummary) { self.0 += summary.len_utf16; }
}
...
impl TextDimension for BufferOffset {
    ...
    fn from_chunk(chunk: rope::ChunkSlice) -> Self { BufferOffset(usize::from_chunk(chunk)) }
    fn add_assign(&mut self, other: &Self) { TextDimension::add_assign(&mut self.0, &other.0); }
}
...
impl Sub for BufferOffset {
    ...
    fn sub(self, other: BufferOffset) -> Self::Output { self.0 - other.0 }
}

impl AddAssign<DimensionPair<usize, Point>> for BufferOffset {
    fn add_assign(&mut self, other: DimensionPair<usize, Point>) { self.0 += other.key; }
}

impl language::ToPoint for BufferOffset {
    fn to_point(&self, snapshot: &text::BufferSnapshot) -> Point { self.0.to_point(snapshot) }
}
...
impl language::ToOffset for BufferOffset {
    fn to_offset(&self, snapshot: &text::BufferSnapshot) -> usize { self.0.to_offset(snapshot) }
}
...
impl ops::Add<usize> for MultiBufferOffsetUtf16 {
    ...
    fn add(self, rhs: usize) -> Self::Output { MultiBufferOffsetUtf16(OffsetUtf16(self.0.0 + rhs)) }
}

impl ops::Add<OffsetUtf16> for MultiBufferOffsetUtf16 {
    type Output = Self;

    fn add(self, rhs: OffsetUtf16) -> Self::Output { MultiBufferOffsetUtf16(self.0 + rhs) }
}

impl AddAssign<OffsetUtf16> for MultiBufferOffsetUtf16 {
    fn add_assign(&mut self, rhs: OffsetUtf16) { self.0 += rhs; }
}

impl AddAssign<usize> for MultiBufferOffsetUtf16 {
    fn add_assign(&mut self, rhs: usize) { self.0.0 += rhs; }
}

impl Sub for MultiBufferOffsetUtf16 {
    type Output = OffsetUtf16;

    fn sub(self, other: MultiBufferOffsetUtf16) -> Self::Output { self.0 - other.0 }
}

impl Sub<OffsetUtf16> for MultiBufferOffsetUtf16 {
    type Output = MultiBufferOffsetUtf16;

    fn sub(self, other: OffsetUtf16) -> Self::Output { MultiBufferOffsetUtf16(self.0 - other) }
}
...
impl ops::Sub for MultiBufferOffset {
    ...
    fn sub(self, other: MultiBufferOffset) -> Self::Output { self.0 - other.0 }
}

impl ops::Sub<usize> for MultiBufferOffset {
    type Output = Self;

    fn sub(self, other: usize) -> Self::Output { MultiBufferOffset(self.0 - other) }
}
...
impl ops::Add<usize> for BufferOffset {
    ...
    fn add(self, rhs: usize) -> Self::Output { BufferOffset(self.0 + rhs) }
}
...
impl ops::Add<usize> for MultiBufferOffset {
    ...
    fn add(self, rhs: usize) -> Self::Output { MultiBufferOffset(self.0 + rhs) }
}
...
impl ops::Add<isize> for MultiBufferOffset {
    ...
    fn add(self, rhs: isize) -> Self::Output { MultiBufferOffset((self.0 as isize + rhs) as usize) }
}

impl ops::Add for MultiBufferOffset {
    type Output = Self;

    fn add(self, rhs: MultiBufferOffset) -> Self::Output { MultiBufferOffset(self.0 + rhs.0) }
}

impl ops::AddAssign<MultiBufferOffset> for MultiBufferOffset {
    fn add_assign(&mut self, other: MultiBufferOffset) { self.0 += other.0; }
}
...
impl std::ops::Deref for DiffStateSnapshot {
    ...
    fn deref(&self) -> &Self::Target { &self.diff }
}
...
impl sum_tree::KeyedItem for DiffStateSnapshot {
    ...
    fn key(&self) -> Option<BufferId> { Some(self.buffer_id) }
}

impl<'a> Dimension<'a, DiffStateSummary> for Option<BufferId> {
    fn zero(_cx: ()) -> Self { None }
...
impl ExcerptBoundaryInfo {
    pub fn start_text_anchor(&self) -> text::Anchor { self.range.context.start }
    pub fn buffer_id(&self) -> BufferId { self.start_text_anchor().buffer_id }
...
impl ExcerptSummary {
    ...
    fn len(&self) -> ExcerptOffset { ExcerptDimension(self.text.len) }
}
...
impl From<&str> for MBTextSummary {
    fn from(text: &str) -> Self { MBTextSummary::from(TextSummary::from(text)) }
}

impl MultiBufferDimension for MBTextSummary {
    type TextDimension = TextSummary;

    fn from_summary(summary: &MBTextSummary) -> Self { *summary }

    fn add_text_dim(&mut self, summary: &Self::TextDimension) { *self += *summary; }

    fn add_mb_text_summary(&mut self, summary: &MBTextSummary) { *self += *summary; }
}
...
impl AddAssign<TextSummary> for MBTextSummary {
    fn add_assign(&mut self, other: TextSummary) { *self += MBTextSummary::from(other); }
}
...
impl IndentGuide {
    pub fn indent_level(&self) -> u32 { self.depth * self.tab_size }
}
...
impl MultiBuffer {
    ...
    pub fn is_dirty(&self, cx: &App) -> bool { self.read(cx).is_dirty() }

    pub fn has_deleted_file(&self, cx: &App) -> bool { self.read(cx).has_deleted_file() }

    pub fn has_conflict(&self, cx: &App) -> bool { self.read(cx).has_conflict() }

    // The `is_empty` signature doesn't match what clippy expects
    #[allow(clippy::len_without_is_empty)]
    pub fn len(&self, cx: &App) -> MultiBufferOffset { self.read(cx).len() }
    ...
    pub fn all_buffers(&self) -> HashSet<Entity<Buffer>> { self.all_buffers_iter().collect() }
    ...
    pub fn explicit_title(&self) -> Option<&str> { self.title.as_deref() }
    ...
    pub fn all_diff_hunks_expanded(&self) -> bool { self.snapshot.borrow().all_diff_hunks_expanded }
...
impl MultiBuffer {
    ...
    fn check_invariants(&self, cx: &App) { self.read(cx).check_invariants(); }
}
...
impl MultiBufferSnapshot {
    ...
    pub fn has_diff_hunks(&self) -> bool { self.diffs.iter().any(|diff| !diff.is_empty()) }
    ...
    pub fn len(&self) -> MultiBufferOffset { self.diff_transforms.summary().output.len }
    ...
    pub fn max_point(&self) -> Point { self.text_summary().lines }

    pub fn max_row(&self) -> MultiBufferRow { MultiBufferRow(self.text_summary().lines.row) }

    pub fn text_summary(&self) -> MBTextSummary { self.diff_transforms.summary().output }
    ...
    pub fn edit_count(&self) -> usize { self.edit_count }
    ...
    pub fn trailing_excerpt_update_count(&self) -> usize { self.trailing_excerpt_update_count }
    ...
    pub fn has_deleted_file(&self) -> bool { self.has_deleted_file }

    pub fn has_conflict(&self) -> bool { self.has_conflict }
    ...
    pub fn show_headers(&self) -> bool { self.show_headers }
    ...
    BD: TextDimension + AddAssign<<MBD as Sub>::Output>,
{
    ...
    fn excerpt(&self) -> Option<&'a Excerpt> { self.excerpts.item() }
}
...
impl DiffTransformSummary {
    fn excerpt_len(&self) -> ExcerptOffset { ExcerptDimension(self.input.len) }
}
...
impl sum_tree::Dimension<'_, ExcerptSummary> for PathKey {
    fn zero(_: <ExcerptSummary as sum_tree::Summary>::Context<'_>) -> Self { PathKey::min() }
...
{
    fn zero(_: ()) -> Self { ExcerptDimension(MBD::default()) }
...
impl<T: PartialOrd> PartialOrd<T> for OutputDimension<T> {
    fn partial_cmp(&self, other: &T) -> Option<cmp::Ordering> { self.0.partial_cmp(other) }
}
    ...
    T: ops::Sub<U, Output = R>,
{
    ...
    fn sub(self, other: OutputDimension<U>) -> Self::Output { self.0 - other.0 }
}
    ...
    T: ops::Add<U, Output = R>,
{
    ...
    fn add(self, other: U) -> Self::Output { OutputDimension(self.0 + other) }
}
...
where
    T: AddAssign<U>,
{
    fn add_assign(&mut self, other: U) { self.0 += other; }
}
...
where
    T: SubAssign<U>,
{
    fn sub_assign(&mut self, other: U) { self.0 -= other; }
}
...
impl ExcerptOffset {
    fn saturating_sub(self, other: ExcerptOffset) -> usize { self.0.saturating_sub(other.0) }
}
    ...
    T: ops::Sub<U, Output = R>,
{
    ...
    fn sub(self, other: ExcerptDimension<U>) -> Self::Output { self.0 - other.0 }
}
    ...
    T: ops::Add<U, Output = R>,
{
    ...
    fn add(self, other: U) -> Self::Output { ExcerptDimension(self.0 + other) }
}
...
impl<'a> sum_tree::Dimension<'a, DiffTransformSummary> for MultiBufferOffset {
    fn zero(_: ()) -> Self { MultiBufferOffset::ZERO }
...
impl<'a, MBD: MultiBufferDimension> sum_tree::Dimension<'a, DiffTransformSummary>
    for OutputDimension<MBD>
{
    fn zero(_: ()) -> Self { OutputDimension(MBD::default()) }
...
impl<'a> MultiBufferChunks<'a> {
    pub fn offset(&self) -> MultiBufferOffset { self.range.start }
...
impl ToOffset for MultiBufferOffsetUtf16 {
    ...
    fn to_offset_utf16(&self, _snapshot: &MultiBufferSnapshot) -> MultiBufferOffsetUtf16 { *self }
}
...
impl ToPoint for Point {
    fn to_point<'a>(&self, _: &MultiBufferSnapshot) -> Point { *self }
    fn to_point_utf16<'a>(&self, snapshot: &MultiBufferSnapshot) -> PointUtf16 {
...
impl ToPoint for PointUtf16 {
    ...
    fn to_point_utf16<'a>(&self, _: &MultiBufferSnapshot) -> PointUtf16 { *self }
}
