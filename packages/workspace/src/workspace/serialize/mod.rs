...
impl Workspace {
    //  flush_serialization / remove_from_session
    ...
    // 把「当前 workspace 该用什么身份被持久化」归结成一个枚举——供序列化流程（serialize_workspace_internal）决定往数据库里写什么、怎么写
    fn workspace_location(&self, cx: &App) -> WorkspaceLocation {
        // 1. 收集这个 workspace 所有可见 worktree 的绝对路径
        let paths = PathList::new(&self.root_paths(cx));
        // 2. 三种身份判定，按优先级排列
        if let Some(connection) = self.project.read(cx).remote_connection_options(cx) {
            // 有远程连接配置 → 这是个远程（SSH/WSL/Docker）项目
    ...
    // 把当前窗口的几何状态写进数据库，让下次启动时能恢复窗口位置和大小。写法分两条路
