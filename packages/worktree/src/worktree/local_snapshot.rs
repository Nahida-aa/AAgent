...
pub struct LocalSnapshot {
    ...
    pub(crate) global_gitignore: Option<Arc<Gitignore>>,
    ...
    pub(crate) repo_exclude_by_work_dir_abs_path: HashMap<Arc<Path>, (Arc<Gitignore>, bool)>,
    /// All of the gitignore files in the worktree, indexed by their absolute path.
    /// The boolean indicates whether the gitignore needs to be updated.
    pub(crate) ignores_by_parent_abs_path: HashMap<Arc<Path>, (Arc<Gitignore>, bool)>,
    /// All of the git repositories in the worktree, indexed by the project entry
    /// id of their parent directory.
    pub(crate) git_repositories: TreeMap<ProjectEntryId, LocalRepositoryEntry>,
    /// The file handle of the worktree root
    /// (so we can find it after it's been moved)
    pub(crate) root_file_handle: Option<Arc<dyn fs::FileHandle>>,
    ...
    pub(crate) external_canonical_to_relative: BTreeMap<Arc<Path>, Arc<RelPath>>,
}

impl LocalSnapshot {
    pub(crate) fn local_repo_for_work_directory_path(
        &self,
        path: &RelPath,
    ...
    pub(crate) async fn insert_entry(&mut self, mut entry: Entry, fs: &dyn Fs) -> Entry {
