...
impl RemoteConnectionPrompt {
    ...
    pub fn set_cancellation_tx(&mut self, tx: oneshot::Sender<()>) { self.cancellation = Some(tx); }
...
impl remote::RemoteClientDelegate for RemoteClientDelegate {
    ...
    fn set_status(&self, status: Option<&str>, cx: &mut AsyncApp) { self.update_status(status, cx) }
