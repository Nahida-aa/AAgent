...
pub fn init(cx: &mut App) { registry::init(cx); }
...
impl DisabledReason {
    pub fn new(reason: impl Into<SharedString>) -> Self { Self(reason.into()) }
}
...
pub trait LanguageModel: Send + Sync {
    ...
    fn upstream_provider_id(&self) -> LanguageModelProviderId { self.provider_id() }
    fn upstream_provider_name(&self) -> LanguageModelProviderName { self.provider_name() }

    /// Returns whether this model is the "latest", so we can highlight it in the UI.
    fn is_latest(&self) -> bool { false }

    /// Whether the model is currently disabled and, if so, why this is the case.
    fn is_disabled(&self) -> Option<DisabledReason> { None }
    ...
    fn requires_data_retention(&self) -> bool { false }

    /// When this model refuses a request, the model ID to fall back to (same provider).
    fn refusal_fallback_model_id(&self) -> Option<&'static str> { None }
    ...
    fn supports_disabling_thinking(&self) -> bool { true }
    ...
    fn supports_server_side_compaction(&self) -> bool { false }

    fn supports_explicit_compaction(&self) -> bool { false }

    /// Whether native compaction honors `LanguageModelRequest::max_output_tokens`.
    fn supports_explicit_compaction_output_limit(&self) -> bool { false }

    /// The provider-enforced input size required for explicit compaction.
    fn minimum_explicit_compaction_input_tokens(&self) -> Option<u64> { None }
    ...
    fn max_input_tokens(&self) -> u64 { self.max_token_count() }
    ...
    fn max_total_tokens(&self) -> Option<u64> { Some(self.max_token_count()) }
...
impl Default for IconOrSvg {
    fn default() -> Self { Self::Icon(IconName::ZedAssistant) }
}

pub trait LanguageModelProvider: 'static {
    fn id(&self) -> LanguageModelProviderId;
    fn name(&self) -> LanguageModelProviderName;
    fn icon(&self) -> IconOrSvg { IconOrSvg::default() }
    fn default_model(&self, cx: &App) -> Option<Arc<dyn LanguageModel>>;
    fn default_fast_model(&self, cx: &App) -> Option<Arc<dyn LanguageModel>>;
    fn provided_models(&self, cx: &App) -> Vec<Arc<dyn LanguageModel>>;
    fn recommended_models(&self, _cx: &App) -> Vec<Arc<dyn LanguageModel>> { Vec::new() }
    ...
    fn fast_mode_confirmation(&self, _cx: &App) -> Option<FastModeConfirmation> { None }
}
