...
impl DapRegistry {
    pub fn global(cx: &mut App) -> &mut Self { cx.default_global::<Self>() }
    ...
    pub fn remove_adapter(&self, name: &str) { self.0.write().adapters.remove(name); }

    pub fn remove_locator(&self, locator: &str) { self.0.write().locators.remove(locator); }
