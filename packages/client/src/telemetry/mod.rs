...
impl Telemetry {
    ...
    fn shutdown_telemetry(self: &Arc<Self>) -> impl Future<Output = ()> + use<> { Task::ready(()) }
    ...
    pub fn log_file_path() -> PathBuf { paths::logs_dir().join("telemetry.log") }
    ...
    pub fn has_checksum_seed(&self) -> bool { ZED_CLIENT_CHECKSUM_SEED.is_some() }
    ...
    pub fn metrics_enabled(self: &Arc<Self>) -> bool { self.state.lock().settings.metrics }

    pub fn diagnostics_enabled(self: &Arc<Self>) -> bool { self.state.lock().settings.diagnostics }
    ...
    pub fn metrics_id(self: &Arc<Self>) -> Option<Arc<str>> { self.state.lock().metrics_id.clone() }

    pub fn system_id(self: &Arc<Self>) -> Option<Arc<str>> { self.state.lock().system_id.clone() }

    pub fn installation_id(self: &Arc<Self>) -> Option<Arc<str>> {
        self.state.lock().installation_id.clone()
    }

    pub fn is_staff(self: &Arc<Self>) -> Option<bool> { self.state.lock().is_staff }
