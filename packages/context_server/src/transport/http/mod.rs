...
impl Transport for HttpTransport {
    async fn send(&self, message: String) -> Result<()> { self.send_message(message).await }
    ...
    fn auth_challenge(&self) -> Option<WwwAuthenticate> { self.auth_challenge.lock().clone() }
}
...
mod tests {
    ...
    impl FakeTokenProvider {
        ...
        fn set_token(&self, token: &str) { *self.token.lock() = Some(token.to_string()); }

        fn refresh_count(&self) -> usize { self.refresh_count.load(Ordering::SeqCst) }
    }

    #[async_trait]
    impl OAuthTokenProvider for FakeTokenProvider {
        fn access_token(&self) -> Option<String> { self.token.lock().clone() }
