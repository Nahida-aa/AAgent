...
{
    fn from(client: Arc<T>) -> Self { Self::new(client) }
}
...
impl AnyProtoClient {
    ...
    pub fn is_via_collab(&self) -> bool { self.0.client.is_via_collab() }
    ...
    pub fn has_wsl_interop(&self) -> bool { self.0.client.has_wsl_interop() }
}
...
impl ProtoClient for NoopProtoClient {
    ...
    fn send(&self, _: proto::Envelope, _: &'static str) -> Result<()> { Ok(()) }
    fn send_response(&self, _: proto::Envelope, _: &'static str) -> Result<()> { Ok(()) }
    fn message_handler_set(&self) -> &parking_lot::Mutex<ProtoMessageHandlerSet> {
        &self.handler_set
    }
