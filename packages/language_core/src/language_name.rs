...
impl LanguageId {
    pub fn new() -> Self { Self(NEXT_LANGUAGE_ID.fetch_add(1, SeqCst)) }
}
...
impl LanguageName {
    pub fn new(s: &str) -> Self { Self(SharedString::new(s)) }

    pub fn new_static(s: &'static str) -> Self { Self(SharedString::new_static(s)) }

    pub fn from_proto(s: String) -> Self { Self(SharedString::from(s)) }

    pub fn to_proto(&self) -> String { self.0.to_string() }
    ...
    pub fn snippet_scope_id(&self) -> String { self.lsp_id().replace(['/', '\\'], "") }
}

impl From<LanguageName> for SharedString {
    fn from(value: LanguageName) -> Self { value.0 }
}

impl From<SharedString> for LanguageName {
    fn from(value: SharedString) -> Self { LanguageName(value) }
}
...
impl From<&'static str> for LanguageName {
    fn from(str: &'static str) -> Self { Self(SharedString::new_static(str)) }
}
