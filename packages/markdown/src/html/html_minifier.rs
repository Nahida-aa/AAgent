...
impl<'a> Context<'a> {
    ...
    fn parent_trim_left(&self) -> bool { self.parent_context.map_or(true, Context::trim_left) }
...
fn is_ascii_whitespace(c: char) -> bool { c.is_ascii_whitespace() }

fn preserve_whitespace(name: &str) -> bool { matches!(name, "pre" | "textarea") }

fn contains_code(name: &str) -> bool { matches!(name, "script" | "style") }
