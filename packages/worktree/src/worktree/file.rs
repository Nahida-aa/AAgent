...
impl language::File for File {
    ...
    fn disk_state(&self) -> DiskState { self.disk_state }

    fn path(&self) -> &Arc<RelPath> { &self.path }

    fn full_path(&self, cx: &App) -> PathBuf { self.worktree.read(cx).full_path(&self.path) }
    ...
    fn worktree_id(&self, cx: &App) -> WorktreeId { self.worktree.read(cx).id() }
    ...
    fn path_style(&self, cx: &App) -> PathStyle { self.worktree.read(cx).path_style() }

    fn can_open(&self) -> bool { true }
}

impl language::LocalFile for File {
    fn abs_path(&self, cx: &App) -> PathBuf { self.worktree.read(cx).absolutize(&self.path) }
...
impl File {
    ...
    pub fn worktree_id(&self, cx: &App) -> WorktreeId { self.worktree.read(cx).id() }
