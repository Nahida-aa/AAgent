...
pub(super) struct FocusablePart {
...
pub(super) struct RegionFocusHandles {
...
impl Workspace {
    ...
    pub(super) fn move_titlebar_item_focus(
        &mut self,
        forward: bool,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    pub(super) fn focusable_parts(&self, cx: &App) -> Vec<FocusablePart> {
    ...
    pub(super) fn fallback_focus_handle(&self, window: &Window, cx: &App) -> FocusHandle {
        self.all_docks()
            .into_iter()
            .find_map(|dock| {
                let dock = dock.read(cx);
    ...
    pub(super) fn focus_center_pane(&mut self, window: &mut Window, cx: &mut Context<Self>) {
