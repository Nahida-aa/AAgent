...
impl JsonSchema for KeymapAction {
    ...
    fn json_schema(_: &mut schemars::SchemaGenerator) -> schemars::Schema { json_schema!(true) }
}
...
impl JsonSchema for UnbindTargetAction {
    fn schema_name() -> Cow<'static, str> { "UnbindTargetAction".into() }
...
impl KeymapFile {
    ...
    pub fn sections(&self) -> impl DoubleEndedIterator<Item = &KeymapSection> { self.0.iter() }
...
impl<'a> KeybindUpdateOperation<'a> {
    pub fn add(source: KeybindUpdateTarget<'a>) -> Self { Self::Add { source, from: None } }
}
...
impl From<KeyBindingMetaIndex> for KeybindSource {
    fn from(index: KeyBindingMetaIndex) -> Self { Self::from_meta(index) }
}

impl From<KeybindSource> for KeyBindingMetaIndex {
    fn from(source: KeybindSource) -> Self { source.meta() }
}
...
impl Action for ActionSequence {
    fn name(&self) -> &'static str { Self::name_for_type() }
