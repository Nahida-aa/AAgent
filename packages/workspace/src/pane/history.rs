...
use gpui::{App, Context, EntityId, WeakEntity};
...
pub struct ItemNavHistory {
    pub(super) history: NavHistory,
    pub(super) item: Arc<dyn WeakItemHandle>,
}
...
impl NavHistory {
    ...
    pub fn set_mode(&mut self, mode: NavigationMode) { self.0.lock().mode = mode; }

    pub fn mode(&self) -> NavigationMode { self.0.lock().mode }

    pub fn disable(&mut self) { self.0.lock().mode = NavigationMode::Disabled; }

    pub fn enable(&mut self) { self.0.lock().mode = NavigationMode::Normal; }
