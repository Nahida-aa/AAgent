...
impl Borrow<SharedString> for DebugAdapterName {
    fn borrow(&self) -> &SharedString { &self.0 }
}
...
impl From<DebugAdapterName> for SharedString {
    fn from(name: DebugAdapterName) -> Self { name.0 }
}
impl From<SharedString> for DebugAdapterName {
    fn from(name: SharedString) -> Self { DebugAdapterName(name) }
}

impl<'a> From<&'a str> for DebugAdapterName {
    fn from(str: &'a str) -> DebugAdapterName { DebugAdapterName(str.to_string().into()) }
}
...
pub trait DebugAdapter: 'static + Send + Sync {
    ...
    fn adapter_language_name(&self) -> Option<LanguageName> { None }
    ...
    fn compact_child_session(&self) -> bool { false }

    fn prefer_thread_name(&self) -> bool { false }
}
...
impl DebugAdapter for FakeAdapter {
    fn name(&self) -> DebugAdapterName { DebugAdapterName(Self::ADAPTER_NAME.into()) }

    fn dap_schema(&self) -> serde_json::Value { serde_json::Value::Null }
