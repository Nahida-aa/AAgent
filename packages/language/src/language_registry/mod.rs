...
impl LanguageRegistry {
    ...
    pub fn reload(&self) { self.state.write().reload(); }
    ...
    pub fn subscribe(&self) -> watch::Receiver<()> { self.state.read().subscription.1.clone() }

    /// Returns the number of times that the registry has been changed,
    /// by adding languages or reloading.
    pub fn version(&self) -> usize { self.state.read().version }

    /// Returns the number of times that the registry has been reloaded.
    pub fn reload_count(&self) -> usize { self.state.read().reload_count }
    ...
    pub fn to_vec(&self) -> Vec<Arc<Language>> { self.state.read().languages.to_vec() }
...
impl LanguageRegistryState {
    ...
    fn mark_language_loaded(&mut self, id: LanguageId) { self.available_languages.mark_loaded(id); }
}
