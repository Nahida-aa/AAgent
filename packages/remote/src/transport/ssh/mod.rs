...
impl Default for SshConnectionHost {
    fn default() -> Self { Self::Hostname(Default::default()) }
}
...
impl AsRef<Child> for MasterProcess {
    fn as_ref(&self) -> &Child { &self.process }
}

impl AsMut<Child> for MasterProcess {
    fn as_mut(&mut self) -> &mut Child { &mut self.process }
}
...
impl RemoteConnection for SshRemoteConnection {
    ...
    fn has_been_killed(&self) -> bool { self.killed.load(Ordering::Acquire) }

    fn connection_options(&self) -> RemoteConnectionOptions {
        RemoteConnectionOptions::Ssh(self.socket.connection_options.clone())
    }

    fn shell(&self) -> String { self.ssh_shell.clone() }

    fn default_system_shell(&self) -> String { self.ssh_default_system_shell.clone() }
    ...
    fn path_style(&self) -> PathStyle { self.ssh_path_style }

    fn remote_platform(&self) -> RemotePlatform { self.ssh_platform }

    fn remote_os_version(&self) -> Option<String> { self.ssh_os_version.clone() }
...
impl SshRemoteConnection {
    ...
    async fn is_sftp_available() -> bool { which::which("sftp").is_ok() }
}
