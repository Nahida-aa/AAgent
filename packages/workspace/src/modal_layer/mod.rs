...
impl<V: ModalView> ModalViewHandle for Entity<V> {
    ...
    fn view(&self) -> AnyView { self.clone().into() }
    ...
    fn fade_out_background(&self, cx: &mut App) -> bool { self.read(cx).fade_out_background() }

    fn render_bare(&self, cx: &mut App) -> bool { self.read(cx).render_bare() }
}
...
impl ModalLayer {
    ...
    pub fn has_active_modal(&self) -> bool { self.active_modal.is_some() }
}
