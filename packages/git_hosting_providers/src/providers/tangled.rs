...
impl Tangled {
    ...
    /// with a `did:` scheme.
    fn join_path(&self, path: &str) -> Url { self.base_url().join(&format!("/{path}")).unwrap() }
}

impl GitHostingProvider for Tangled {
    fn name(&self) -> String { self.name.clone() }

    fn base_url(&self) -> Url { self.base_url.clone() }

    fn supports_avatars(&self) -> bool { false }

    fn format_line_number(&self, line: u32) -> String { format!("L{line}") }
