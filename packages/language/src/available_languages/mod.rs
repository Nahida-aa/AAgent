...
impl AvailableLanguage {
    ...
    pub fn name(&self) -> LanguageName { self.name.clone() }

    pub fn matcher(&self) -> &LanguageMatcher { &self.matcher }
...
impl AvailableLanguages {
    ...
    pub(super) fn add(&mut self, language: AvailableLanguage) { self.0.push(language); }
