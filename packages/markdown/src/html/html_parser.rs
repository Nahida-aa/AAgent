...
impl HtmlImage {
    ...
    fn set_alt_text(&mut self, alt_text: SharedString) { self.alt_text = Some(alt_text); }

    fn set_width(&mut self, width: DefiniteLength) { self.width = Some(width); }

    fn set_height(&mut self, height: DefiniteLength) { self.height = Some(height); }
}
...
impl Default for ParseHtmlNodeContext {
    fn default() -> Self { Self { list_item_depth: 1 } }
}
